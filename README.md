...
截至 2026 年 8 月 18 日，使用 `docs/rag_accuracy_demo/` 中已完成入库的六份资料进行 100 次测试。测试覆盖差旅制度、采购流程、项目上线手册、产品与账号 FAQ、费用标准表和企业内部信息总表，并包含产品编号、员工编号等结构化查询。

| 测试项 | 结果 |
| --- | --- |
| 测试接口 | `POST /api/v1/retrieval/search/hybrid` |
| 测试请求数 | 100 |
| 并发数 | 5 |
| `top_k` | 5 |
| 最小耗时 | 489.70 ms |
| 平均耗时 | 590.11 ms |
| P95 | **686.09 ms（0.686 秒）** |
| P99 | 717.18 ms |
| 最大耗时 | 725.23 ms |
| 错误请求 | 0 |
| 空结果请求 | 0 |

本次 P95 为 **0.686 秒**，低于 1.8 秒，达到“优秀”标准。上述数据统计的是混合检索接口从请求到返回结果的耗时，不包含大模型最终回答生成耗时。
